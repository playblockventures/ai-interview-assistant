...
// ─────────────────────────────────────────────────────────────────────────────
// Searchable dropdown — replaces native <select> with an inline search input
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <div ref={containerRef} style={{ position: 'relative', ...style }}>
      ...
      {open && (
        <div style={{
          ...
          maxHeight: 220, overflow: 'hidden', display: 'flex', flexDirection: 'column', marginTop: 2,
        }}>
...
function CandidateModal({ onClose, onSaved, initial = null }) {
  ...
  const extractFromLinkedIn = async () => {
    ...
    try {
      ...
      if (data.resumeText) setResumeText(data.resumeText);
      if (data.linkedinProfile) {
        const lp = data.linkedinProfile;
        const exps = (lp.experiences || lp.experience || []).slice(0, 3);
        setLinkedinProfile({ ...lp, experiences: exps, experience: undefined });
      }
      setShowLinkedInSection(true);
