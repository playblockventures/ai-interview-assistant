...
import { candidateApi, generateApi, interviewApi, notificationApi, settingsApi } from '../utils/api';
...
function ConversationTab({ candidate, appliedScenario, onStatusChange }) {
  ...
  // Call done state — local, initialized from candidate prop
  const [callDone,        setCallDone]        = useState(!!candidate?.callDone);
  const [callDoneAt,      setCallDoneAt]      = useState(candidate?.callDoneAt || null);
  ...
  const handleToggleCallDone = async () => {
    ...
    try {
      if (callDone) {
        await candidateApi.unmarkCallDone(candidate.id);
        setCallDone(false);
        setCallDoneAt(null);
        toast.success('Call marked as not done');
      } else {
        await candidateApi.markCallDone(candidate.id);
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        {callDone && (
                          <span style={{ fontSize: 11, fontWeight: 600, color: '#10b981', background: 'rgba(16,185,129,0.12)', padding: '2px 8px', borderRadius: 8, border: '1px solid rgba(16,185,129,0.3)' }}>
                            ✓ Done {callDoneAt ? `· ${new Date(callDoneAt).toLocaleDateString([], { month: 'short', day: 'numeric' })}` : ''}
                          </span>
                        )}
                      </div>
                        ...
                        {editingIdx === i ? (
                          ...
                          <>
                            <button className="btn btn-secondary btn-sm" onClick={handleToggleCallDone} disabled={callDoneLoading}
                              style={callDone ? { borderColor: '#10b981', color: '#10b981' } : {}}
                              title={callDone ? 'Unmark call as done' : 'Mark call as done'}>
                              {callDoneLoading ? <span className="spinner" style={{ width: 12, height: 12 }} /> : callDone ? '✓ Done' : '☎ Mark Done'}
                            </button>
